...
extern pthread_t	thread_ipcmsg;
//extern pthread_t	thread_mouse;
//extern pthread_t	thread_kb;
...
)
{
	...
	AddRectClipRegion (_lGUI_pAppStat->pClipRgn, &pWin->rect);
	GetBoundClipRegion(_lGUI_pAppStat->pClipRgn);

	//send create application message to server
	//application name is PID
	sprintf(_lGUI_pAppStat->pAppName,"%d",getpid());
	CopyRect(&_lGUI_pAppStat->rc,&pWin->rect);

	SendMsgByClient(LMSG_IPC_CREATEAPP,(char*)_lGUI_pAppStat,sizeof(lGUIAppStat));
...
)
{
	...
	if(_lGUI_pAppStat){
		...
		while(_lGUI_pAppStat){
			SendSigTerm((pid_t)(_lGUI_pAppStat->pAppName));
			//SendMsgByServer(pStat,LSMG_IPC_EXITSERVER,NULL,0);
			EmptyClipRegion(_lGUI_pAppStat->pClipRgn);
			free(_lGUI_pAppStat->pClipRgn);
			close(_lGUI_pAppStat->fdSocket);
			pthread_cancel(_lGUI_pAppStat->tdSocket);
			pthread_join(_lGUI_pAppStat->tdSocket,NULL);
...
)
{
	...
	if(!bByParent){
		pthread_cancel(pWin->threadid);
		pthread_join(pWin->threadid,NULL);
	}
