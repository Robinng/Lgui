...
int _lGUI_iTotalTimer=0;

PTimerLink _lGUI_pTimerHead =NULL;
PTimerLink _lGUI_pTimerTail =NULL;
...
)
{
	...
	if(!pLink)
		return false;
	if((_lGUI_pTimerHead = _lGUI_pTimerTail) == NULL){
		_lGUI_pTimerHead = _lGUI_pTimerTail = pLink;
	}
	else{
		pLink->pNext = NULL;
		pLink->pPrev = _lGUI_pTimerTail;
		_lGUI_pTimerTail->pNext = pLink;
		_lGUI_pTimerTail = pLink;
	}
	pLink->hWnd = hWnd;
	pLink->id = id;
...
)
{
	...
	if(_lGUI_pTimerHead != _lGUI_pTimerTail){
		if(pLink == _lGUI_pTimerHead){
			pLink->pNext->pPrev = NULL;
			_lGUI_pTimerHead = pLink->pNext;
		}
		else if(pLink == _lGUI_pTimerTail){
			pLink->pPrev->pNext = NULL;
			_lGUI_pTimerTail = pLink->pPrev;
		}
		else{
		...
		_lGUI_pTimerHead = _lGUI_pTimerTail = NULL;
	}
	free(pLink);
...
UnInitTimer()
{
	PTimerLink pLink;
	EndTimer();
	while(_lGUI_pTimerHead){
		pLink=_lGUI_pTimerHead->pNext;
		free(_lGUI_pTimerHead);
		_lGUI_pTimerHead=pLink;
	}
}
