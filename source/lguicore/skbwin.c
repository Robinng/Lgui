...
)
{
	...
	switch(iMsg)
	{
		...
		case LMSG_PAINT:
			...
			iSize = SKBWIN_WIDTH * SKBWIN_HEIGHT * _lGUI_iBytesDataType;
...
)
{
	...
	case SKB_LAYOUT_STANDARD:
		return pData;
	case SKB_LAYOUT_SHIFTED:
		return pData+SKBWIN_WIDTH*SKBWIN_HEIGHT*_lGUI_iBytesDataType;
	case SKB_LAYOUT_STANDARD_EXTEND:
		return pData+SKBWIN_WIDTH*SKBWIN_HEIGHT*_lGUI_iBytesDataType * 2;
	case SKB_LAYOUT_SHIFTED_EXTEND:
		return pData+SKBWIN_WIDTH*SKBWIN_HEIGHT*_lGUI_iBytesDataType * 3;
	case SKB_LAYOUT_HANDWRITE:
		return pData+SKBWIN_WIDTH*SKBWIN_HEIGHT*_lGUI_iBytesDataType * 4;
	}
}
...
)
{
	...
	pData+=(lpRect->top*SKBWIN_WIDTH + lpRect->left)*_lGUI_iBytesDataType;

	for(y=0;y<iHeight;y++){
		for(x=0; x<iWidth * _lGUI_iBytesDataType;x++){
			*(pData+x) = ~(*(pData+x));
		}
		pData += SKBWIN_WIDTH * _lGUI_iBytesDataType;
	}
...
)
{
	...
	if(!pWin)
		return false;
	pKeyBoard = (PLGUIKeyBoard)malloc(sizeof(LGUIKeyBoard));
	if(!pKeyBoard)
		return false; 
...
//====================================================================
	...
	iSize = SKBWIN_WIDTH*SKBWIN_HEIGHT*_lGUI_iBytesDataType;
	pData = (char*)malloc(SKB_LAYOUT_NUMBER*iSize);
	if(!pData)
		return false;
	...
	if(!fp)
		return false;
	ReadBmpToBuf(fp,SKBWIN_WIDTH,SKBWIN_HEIGHT,pData);
	fclose(fp);
	pData+=iSize;
